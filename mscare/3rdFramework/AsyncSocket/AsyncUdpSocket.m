...
- (void)doSend:(CFSocketRef)theSocket
{
	if(theCurrentSend != nil)
	{
		...
		if([self canAcceptBytes:theSocket])
		{
			...
			unsigned bufSize = (unsigned)[theCurrentSend->buffer length];
			
			if([self isConnected])
			{
				result = (int)send(theNativeSocket, buf, bufSize, 0);
			}
			else
			{
				const void *dst  = [theCurrentSend->address bytes];
				unsigned dstSize = (unsigned)[theCurrentSend->address length];
				
				result = (int)sendto(theNativeSocket, buf, bufSize, 0, dst, dstSize);
			}
		...
		do
		{
			...
			if([self hasBytesAvailable:theSocket])
			{
				...
				if(theSocket == theSocket4)
				{
					...
					result = (int)recvfrom(theNativeSocket, buf, bufSize, 0, (struct sockaddr *)&sockaddr4, &sockaddr4len);
				...
				else
				{
					...
					result = (int)recvfrom(theNativeSocket, buf, bufSize, 0, (struct sockaddr *)&sockaddr6, &sockaddr6len);
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketWriteCallBack:
			...
			break;
		default:
			NSLog (@"AsyncUdpSocket %p received unexpected CFSocketCallBackType %d.", self, (int)type);
			break;
	}
}
